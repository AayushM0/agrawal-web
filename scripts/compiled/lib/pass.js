...
export function formatRoleLabel(relationToHead) {
    if (!relationToHead)
        return "Member";
...
export function transliterateDevanagariWord(word) {
    if (!word)
        return "";
    if (KNOWN_WORDS[word])
        return KNOWN_WORDS[word];
    const cleanWord = word.replace(/[.,/#!$%^&*;:{}=\-_`~()]/g, "");
    if (KNOWN_WORDS[cleanWord])
        return KNOWN_WORDS[cleanWord];
    let result = "";
...
export function toSafeLatinText(input) {
    if (!input)
        return "";
...
export function createUnifiedPassData({ member, household }) {
    const serialNo = member.serialNo ||
        household?.serialNo ||
    ...
    const rawCity = member.currentCity || household?.city || household?.nativePlace || member.nativePlace || "Delhi";
    ...
    const isSpouseOrMarriedFemale = member.maritalStatus === "Married" &&
