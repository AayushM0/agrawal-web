...
test("Checklist 4: Supabase Row-Level Security (RLS) enabled in schema.sql and db.ts", () => {
  const schemaPath = path.join(webRoot, "src/db/schema.sql");
  const schema = fs.readFileSync(schemaPath, "utf8");
  ...
  assert.ok(schema.includes("ALTER TABLE action_rate_limits ENABLE ROW LEVEL SECURITY;"), "action_rate_limits RLS must be enabled in schema.sql");
  assert.ok(schema.includes("ALTER TABLE matrimonial_profiles ENABLE ROW LEVEL SECURITY;"), "matrimonial_profiles RLS must be enabled in schema.sql");
  assert.ok(db.includes("ALTER TABLE action_rate_limits ENABLE ROW LEVEL SECURITY;"), "action_rate_limits RLS must be enabled in db.ts");
  assert.ok(db.includes("ALTER TABLE matrimonial_profiles ENABLE ROW LEVEL SECURITY;"), "matrimonial_profiles RLS must be enabled in db.ts");
